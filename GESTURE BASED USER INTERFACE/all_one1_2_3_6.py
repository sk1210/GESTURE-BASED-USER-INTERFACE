...
def keyboard():
    ...
    alpha1=['q','w','e','r','t','y','u','i']
    alpha2=['a','s','d','f','g','h','j','k']
    alpha3=['z','x','c','v','b','n','m','p']
    alpha4=['o','l','_____','Caps Lock']
    alpha_rect1=[]
    alpha_rect2=[]
    alpha_rect3=[]
    alpha_rect4=[]
    ...
    while (1):
        ...
        for i in xrange(8):
            ...
            font_obj=font.render(alpha1[i] ,True,WHITE)
        ...
        for i in xrange(8):
            temp=pygame.draw.rect(DISPLAY,MAROON,(x1,y1,key_width,key_height))
            font_obj=font.render(alpha2[i] ,True,WHITE)
            font_rect=font_obj.get_rect(centerx=x1+int(key_width/2),centery=y1+int(key_height/2))
            DISPLAY.blit(font_obj,font_rect)
            x1+=(key_width+5)
        ...
        for i in xrange(8):
            temp=pygame.draw.rect(DISPLAY,MAROON,(x1,y1,key_width,key_height))
            font_obj=font.render(alpha3[i] ,True,WHITE)
            font_rect=font_obj.get_rect(centerx=x1+int(key_width/2),centery=y1+int(key_height/2))
            DISPLAY.blit(font_obj,font_rect)
            x1+=(key_width+5)
        ...
        for i in xrange(5):
            if i<2:
                temp=pygame.draw.rect(DISPLAY,MAROON,(x1,y1,key_width,key_height))
                font_obj=font.render(alpha4[i] ,True,WHITE)
                font_rect=font_obj.get_rect(centerx=x1+int(key_width/2),centery=y1+int(key_height/2))
                DISPLAY.blit(font_obj,font_rect)
                x1+=(key_width+5)
                alpha_rect4.append(temp)
            if i==2:
                temp=pygame.draw.rect(DISPLAY,MAROON,(x1,y1,key_width*3,key_height))
                font_obj=font.render(alpha4[i] ,True,WHITE)
                font_rect=font_obj.get_rect(centerx=x1+int(key_width/2),centery=y1+int(key_height/2))
                DISPLAY.blit(font_obj,font_rect)
                x1+=(key_width*3+5)
